...
Após gerarmos os gráficos e analisarmos os 10 conjuntos de dados escolhidos, necessitávamos analisar as métricas de separação linear, utilizando a biblioteca Scikit-Learn da linguagem Python. Para cada base separável, foram analisadas as seguintes métricas: precisão, revocação e f1-escore.


Como os \textit{datasets} separáveis foram selecionados artificialmente, para que pudéssemos demonstrar o funcionamento do programa, todas as métricas, para todos os datasets separáveis, foram iguais a 1. No entanto, algumas instâncias do \textit{Wine} podem apresentar métricas um pouco abaixo desse valor.
